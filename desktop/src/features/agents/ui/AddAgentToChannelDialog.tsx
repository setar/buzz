      ...
      <DialogContent className="max-w-xl overflow-hidden p-0">
        <div className="flex max-h-[85vh] flex-col">
          <DialogHeader className="shrink-0 border-b border-border/60 px-6 py-5 pr-14">
            <DialogTitle>{t("agents.add_agent_to_channel")}</DialogTitle>
            <DialogDescription>
              {t("agents.add_agent_to_channel_description", {
                name: agent?.name ?? t("agents.this_agent"),
              })}
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="min-h-0 flex-1 space-y-5 overflow-y-auto px-6 py-5">
            <div className="space-y-1.5">
              ...
              <p className="text-xs text-muted-foreground">
                {t("agents.only_accessible_channels")}
              </p>
            </div>
            ...
            {isAlreadyMember ? (
              <div className="flex items-center gap-2 rounded-lg border border-border/50 bg-muted/30 px-3 py-2 text-sm text-muted-foreground">
                <span>✓</span>
                <span>{t("agents.already_member_of_channel")}</span>
              </div>
            ) : null}
            ...
            <div className="space-y-1.5">
              ...
              <select
                ...
                value={role}
              >
                <option value="bot">{t("agents.role_bot")}</option>
                <option value="member">{t("agents.role_member")}</option>
                <option value="guest">{t("agents.role_guest")}</option>
                <option value="admin">{t("agents.role_admin")}</option>
              </select>
            </div>

            <div className="rounded-2xl border border-border/70 bg-muted/20 p-4">
              <p className="text-sm font-semibold tracking-tight">
                {t("agents.agent_pubkey")}
              </p>
              <div className="mt-3 flex items-center justify-between gap-3">
                <code className="min-w-0 flex-1 break-all rounded-xl border border-border/70 bg-background/80 px-3 py-2 text-xs">
                  {agent?.pubkey ?? t("agents.no_agent_selected")}
                </code>
                {agent ? (
                  <CopyButton
                    label={t("agents.copy_pubkey")}
                    value={agent.pubkey}
                  />
                ) : null}
              </div>
          ...
          <div className="flex shrink-0 justify-end gap-2 border-t border-border/60 px-6 py-4">
            ...
            >
              {attachAgentMutation.isPending
                ? t("agents.adding")
                : isAlreadyMember
                  ? t("agents.readd_to_channel")
                  : t("agents.add_to_channel")}
            </Button>
          </div>
        </div>
      </DialogContent>
