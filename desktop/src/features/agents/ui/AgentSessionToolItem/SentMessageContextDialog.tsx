      ...
      <DialogContent className="max-w-xl overflow-hidden p-0">
        <div className="flex max-h-[85vh] flex-col">
          <DialogHeader className="px-6 pb-3 pt-5 pr-14">
            <DialogTitle>{t("agents.sent_message_context")}</DialogTitle>
...
function buildSentMessageContextSections({
  ...
  if (preview) {
    sections.push({ title: t("agents.message"), body: preview });
  }
  if (description) {
    sections.push({ title: t("agents.tool"), body: description });
  }
  if (hasArgs) {
    sections.push({
  ...
  if (hasResult) {
    sections.push({
      title: isError ? t("agents.error") : t("agents.result"),
  ...
  if (sections.length === 0) {
    sections.push({
      ...
      body: t("agents.waiting_for_tool_details"),
    });
  }
  return sections;
}
