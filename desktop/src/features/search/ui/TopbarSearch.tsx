...
function getSearchHitContextLabel(
  ...
  return {
    ...
    text: channelName
      ? `${isThread ? t("search.thread") : t("search.message")} ${t("search.in")}`
      : isThread
        ? t("search.thread")
        : t("search.message"),
  };
...
function getSectionTitle(
  ...
  switch (sectionKey) {
    ...
    case "direct-messages":
      return t("search.direct_messages");
    case "people":
      return "People";
    case "agents":
      return "Agents";
    case "messages":
      return t("search.most_relevant");
    case "actions":
      return "Actions";
  }
}
...
export function TopbarSearch({
  ...
  const suggestionActionResults = React.useMemo(() => {
    ...
    if (onBrowseChannels) {
      actions.push({
        ...
        action: {
          id: "browse-channels",
          title: t("search.browse_channels"),
        },
      });
    }
  ...
  const renderSearchResultRow = (result: SearchResult, index: number) => {
    ...
    const messageContextLabel =
      result.kind === "message"
        ? getSearchHitContextLabel(result.hit, channelLookup, channelLabels, t)
        : null;
    const title =
      result.kind === "channel"
        ? channelDisplayName
      ...
      result.kind === "channel"
        ...
        : result.kind === "action"
          ...
          : result.kind === "user"
            ...
            : truncateResultText(result.hit.content, 96, t);
    const trailingLabel =
      result.kind === "channel"
  ...
  const searchResultContent = isShowingSuggestions ? (
    scopeChannel ? null : suggestionResults.length === 0 ? (
      ...
      <div
        aria-label={t("search.recent_activity")}
  ...
  return (
    <div className={cn("relative", className)}>
      <Dialog open={isOpen} onOpenChange={handleSearchOpenChange}>
        <button
          aria-label={t("search.search_everything")}
          className={
            isIconVariant
          ...
          ref={triggerRef}
          title={t("search.search_everything")}
          type="button"
        >
          <Search
            className={
          ...
          {isIconVariant ? null : (
            <>
              ...
              >
                {query || t("search.search_everything")}
              </span>
        ...
        >
          <DialogTitle className="sr-only">
            {scopeLabel ? t("search.search_in", { scope: scopeLabel }) : t("search.search_everything")}
          </DialogTitle>
          <SearchDialogInputRow
